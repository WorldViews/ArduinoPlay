...
function render(){
    ...
    if (!paused) {
        ...
        if (!mouseout && mouseEnable1){
            GPU.setUniformForProgram("force", "u_mouseEnable1", 1.0, "1f");
            GPU.setUniformForProgram("force", "u_mouseCoord1",
                                     [mouseCoordinates1[0]*scale,
                                      mouseCoordinates1[1]*scale], "2f");
            GPU.setUniformForProgram("force", "u_mouseDir1",
                                     [3*(mouseCoordinates1[0]-lastMouseCoordinates1[0])*scale,
                3*(mouseCoordinates1[1]-lastMouseCoordinates1[1])*scale], "2f");

            GPU.setUniformForProgram("force", "u_mouseEnable2", 1.0, "1f");
//            GPU.setUniformForProgram("force", "u_mouseCoord2", [(300+mouseCoordinates2[0])*scale, (25+mouseCoordinates2[1])*scale], "2f");
            GPU.setUniformForProgram("force", "u_mouseCoord2", [(mouseCoordinates2[0])*scale, (25+mouseCoordinates2[1])*scale], "2f");
            GPU.setUniformForProgram("force", "u_mouseDir2", [3*(mouseCoordinates2[0]-lastMouseCoordinates2[0])*scale,
                3*(mouseCoordinates2[1]-lastMouseCoordinates2[1])*scale], "2f");
            
        } else {
            GPU.setUniformForProgram("force", "u_mouseEnable1", 0.0, "1f");
            GPU.setUniformForProgram("force", "u_mouseEnable2", 0.0, "1f");
        }
        ...
        if (!mouseout && mouseEnable1){
            GPU.setUniformForProgram("addMaterial", "u_mouseEnable1", 1.0, "1f");
            GPU.setUniformForProgram("addMaterial", "u_mouseCoord1", mouseCoordinates1, "2f");
            GPU.setUniformForProgram("addMaterial", "u_mouseLength1", Math.sqrt(Math.pow(3*(mouseCoordinates1[0]-lastMouseCoordinates1[0]),2)
                +Math.pow(3*(mouseCoordinates1[1]-lastMouseCoordinates1[1]),2)), "1f");

            GPU.setUniformForProgram("addMaterial", "u_mouseEnable2", 1.0, "1f");
            GPU.setUniformForProgram("addMaterial", "u_mouseCoord2", mouseCoordinates2, "2f");
            GPU.setUniformForProgram("addMaterial", "u_mouseLength2", Math.sqrt(Math.pow(3*(mouseCoordinates2[0]-lastMouseCoordinates2[0]),2)
                +Math.pow(3*(mouseCoordinates2[1]-lastMouseCoordinates2[1]),2)), "1f");
        } else {
            GPU.setUniformForProgram("addMaterial", "u_mouseEnable1", 0.0, "1f");
            GPU.setUniformForProgram("addMaterial", "u_mouseEnable2", 0.0, "1f");
        }
